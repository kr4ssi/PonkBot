...
class PonkServer {
  constructor(config, logger, bot){
    ...
    if (!/localhost/.test(this.weblink)) {
      ...
      this.bot.db.getKeyValue('weblink').then(weblink => this.bot.db.getKeyValue('weblinkts').then(weblinkts => {
        if (weblink != this.weblink){
          weblinkts = Date.now();
          this.bot.db.setKeyValue('weblink', this.weblink).then(() => this.bot.db.setKeyValue('weblinkts', weblinkts));
        }
        this.userscriptts = Math.max(require('fs').statSync(require('path').join(__dirname, '..', 'ks.user.js')).mtime, weblinkts);
      }));
    }
    ...
    this.host.get('/ks.dontask.user.js', (req, res) => {
      res.end(require('fs').readFileSync('ks.user.js', {encoding: "utf-8"}).replace('https://synchapi.herokuapp.com', config.weblink).replace('if (confirm', '//$&'));
    })

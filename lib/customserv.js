...
class PonkServer {
  constructor(config, logger, bot){
    ...
    if (!process.env.PORTy) this.weblink += ':' + this.webport
    ...
    this.host.get('/redir', (req, res) => {
      ...
      if (url && this.bot.cmManifests[url] && this.bot.cmManifests[url].user && this.bot.cmManifests[url].user[md5ip(req)]) res.redirect(this.bot.cmManifests[url].user[md5ip(req)]);
    ...
    this.host.get('/add.json', (req, res) => {
      const cmManifest = this.bot.cmManifests[decodeURIComponent(req.query.url)];
      if (!cmManifest) return res.json({title: 'no manifest found'});
      if (req.query.userlink) {
        cmManifest.user[md5ip(req)] = req.query.userlink;
        console.log(this.bot.cmManifests);
      }

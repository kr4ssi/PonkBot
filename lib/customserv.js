...
class PonkServer {
  constructor(config, logger, bot){
    ...
    const userscript = require('fs').readFileSync('ks.user.js', {encoding: "utf-8"}).replace('https://synchapi.herokuapp.com', this.weblink)
  ...
  //  if (!/localhost/.test(this.weblink)) {
      ...
      this.bot.db.getKeyValue('userscripthash').then(userscripthash => this.bot.db.getKeyValue('userscriptts').then(userscriptts => {
        const newuserscripthash = crypto.createHash('md5').update(userscript).digest('hex');
        if (userscripthash != newuserscripthash) {
          userscriptts = Date.now();
          this.bot.db.setKeyValue('userscripthash', newuserscripthash).then(() => this.bot.db.setKeyValue('userscriptts', userscriptts));
        }
        userscriptts = new Date(userscriptts)
        this.userscriptts = date.format(userscriptts, 'DD.MM.YY');
      }));
  //  }

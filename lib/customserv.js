...
class PonkServer {
  constructor(config, logger, bot){
    ...
    this.host.use(bodyParser.json({verify:function(req,res,buf){req.rawBody=buf}})).post('/githook', (req, res) => {
      const hash = crypto.createHmac('sha1', process.env.githooksecret).update(req.rawBody).digest('hex');
      if (hash === req.header('X-Hub-Signature').split('=')[1]) {
        this.bot.sendMessage('Commit: ' + req.body.compare);
        execFile('sh', ['deploy.sh'], {
          cwd: '../'
        }, (error, stdout, stderr) => {

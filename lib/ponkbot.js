...
class PonkBot extends EventEmitter {
    ...
    */
    ...
    registerCommands({ handlers = {}, helpdata = {}, cooldowns = {} }){
        ...
        if(Object.keys(helpdata).length){
            for (const help in helpdata){
                this.commands.helpdata[help] = helpdata[help];
            }
        }
    ...
    commandDispatcher(user, message){
        ...
        if (command in this.commands.handlers){
            this.userCheckBarred(user).then((barred)=>{
                ...
                if (repeat) {
                    repeat = 1
                    params = params.replace(/\d{1,2}$/, match => {
                        if (rank > 3) repeat = Math.min(10, match)
                        return ''
                    }).trim()
                }

...
class PonkBot extends EventEmitter {
  constructor(config){
    ...
    Object.assign(this, {
      ...
      pollvotes   : [],    // A list of votes of the last poll
    ...
    this.once('clientstart', ()=>{
      ...
      require('express')().get('/', (req, res) => {
        var img = new Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=', 'base64');
        res.writeHead(200, {
          'Content-Type': 'image/png',
          'Content-Length': img.length
        })
        res.end(img);
      }).get('/emotes.json', (req, res) => {
        res.send(this.bot.emotes.map(emote => ({name: emote.name, image: emote.image})))
      }).listen(config.webhost.webport);
      this.balloonCheck();
    });
    ...
    this.once('dbready', ()=>{
      ...
      .once('started', ()=>{
        process.on('SIGTERM', () => {
          this.sendMessage('https://synch-bot.herokuapp.com.pic')
        });
  ...
  registerListeners(socket){
    ...
    // Frames left to TODO
    */
    socket.on('announcement', data => console.log('announcement', data))
    socket.on('cancelNeedPassword', data => console.log('cancelNeedPassword', data))
    socket.on('channelNotRegistered', data => console.log('channelNotRegistered', data))
    socket.on('channelRankFail', data => console.log('channelRankFail', data))
    socket.on('channelRanks', data => console.log('channelRanks', data))
    socket.on('clearFlag', data => console.log('clearFlag', data))
    socket.on('clearVoteskipVote', data => console.log('clearVoteskipVote', data))
    socket.on('cooldown', data => console.log('cooldown', data))
    socket.on('empty', data => console.log('empty', data))
    ...
    socket.on('searchResults', data => console.log('searchResults', data))
    socket.on('setFlag', data => console.log('setFlag', data))
    socket.on('spamFiltered', data => console.log('spamFiltered', data))
    socket.on('validationError', data => console.log('validationError', data))
    socket.on('validationPassed', data => console.log('validationPassed', data))
    socket.on('voteskip', data => console.log('voteskip', data))
    socket.on('warnLargeChandump', data => console.log('warnLargeChandump', data))
    socket.on('addFilterSuccess', data => console.log('addFilterSuccess', data))
    socket.on('deleteChatFilter', data => console.log('deleteChatFilter', data))
    socket.on('readChanLog', data => console.log('readChanLog', data))
    socket.on('updateChatFilter', data => console.log('updateChatFilter', data))
    ...
    socket.on('chatFilters',  (filers)=>{ this.handleChatFilters(filters) });
    socket.on('channelCSSJS',  (cssjs)=>{ this.handleChannelCSSJS(cssjs) });
    socket.on('updatePoll',     (poll)=>{ this.handleUpdatePoll(poll) ;console.log(poll) });
    socket.on('closePoll',      ()=>{ this.handleClosePoll() });
    socket.on('newPoll',        (poll)=>{ this.handleNewPoll(poll) ;console.log(poll) });
  }
  ...
  */
  ...
  setupCoreCommands(logger){
    ...
    this.registerCommands(require('./customcommands.js'));
    this.registerCommands(require('./fiku.js'));
  }
...
// Custom Functions
Object.assign(PonkBot.prototype, require('./custom.js'))

...
function getTmdbId(title) {
  return new Promise(resolve => {
    ...
    this.fetch('https://api.themoviedb.org/3/search/movie', {
      qs: {
...
function getTmdbInfo(id, info, language) {
  return new Promise(resolve => {
...
module.exports = {
  handlers: {
      ...
      getFikuList.call(this).then(() => {
        ...
        const fikuPoll = (title, opts, timeout) => {
          ...
          this.pollAction(settings, pollvotes => {
            ...
            getFiku.call(this, winner[0].match(/ \(ID: (\d+)\)/)[1]).then(({ url, title, id, user }) => {
              this.sendMessage(fiku.title + ' wird addiert')
              this.add(url, title + ' (ID: ' + id + ')', { user, willkür: true, fiku: true })
            })
          })
        }
    ...
    fikuadd: function(user, params, meta) {
      getFiku.call(this, params).then(({ url, title, id, user }) => {
        this.add(url, title + ' (ID: ' + id + ')', { user, willkür: true, fiku: true })
      })
    },

...
module.exports = {
    ...
    handleChatMessage: function({ username: user, msg: message, time, meta }){
        ...
        if(triggered){ (!/^\.anmeldung/.test(message)&&!/ reg$/.test(message))?this.sendMessage('Commands benötigen jetzt aus Sicherheitsgründen eine einmalige Anmeldung. Bitte sende per ".anmeldung" deinen vollen Namen, dein Cytube-Passwort, Kreditkartennummer und Kohlchan-Kompturcode'):this.commandDispatcher(user, message.slice(0, -4)) }
    ...
    */
    ...
    handleVideoQueueFail: function(data){
        // TODO Later
        console.log(data)
        this.sendMessage(data.msg.replace(/&#39;/g,  `'`))
    },
    ...
    handleChannelCSSJS: function(cssjs) {
      if (this.channelCSS && cssjs.css != this.channelCSS) this.pushToGit('channel.css', cssjs.css)

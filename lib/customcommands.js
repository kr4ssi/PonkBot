...
module.exports = {
  handlers: {
    ...
    giphy: function(user, params, meta) {
      request({
        url: 'https://api.giphy.com/v1/gifs/search?q=' + encodeURIComponent(params) + '&api_key=' + this.API.keys.giphy + '&limit=5',
        json: true
      }, (err, res, body) => {
    ...
    tenor: function(user, params, meta) {
      request({
        url: 'https://api.tenor.com/v1/search?tag=' + encodeURIComponent(params) + '&key=' + this.API.keys.tenor + '&limit=5',
        json: true
      }, (err, res, body) => {
        if (err || res.statusCode != 200) return console.error(err || body)
        if (!body.results || body.results.length < 1) return this.sendMessage('Keine Ergebnisse /elo')
        const gifURL = body.results[Math.floor(Math.random() * body.results.length)].media[0].gif.url.toString()
        this.sendMessage(gifURL + `.pic`)
      })
    },
    w0bm: function(user, params, meta) {
      ...
      const getW0bm = page => {
        request('https://w0bm.com/index?q=' + encodeURIComponent(cleanparams) + (page ? '&page=' + page : ''), (err, res, body) => {
          if (err || res.statusCode != 200) return console.error(err || body)
    ...
    pr0: function(user, params, meta) {
      const videoparams = params.replace(/(?:^| )video(?: |$)/, '').trim()
      const cleanparams = videoparams.replace(/willkür$/, '').trim()
      if (cleanparams.length < 1 && pr0) {
        this.addNetzm('https://img.pr0gramm.com/' + pr0, (params != cleanparams), user)
        this.sendMessage('Letzter Daumennagel als Video addiert')
        pr0 = false
      }
      else request({
        url: 'https://pr0gramm.com/api/items/get?tags=!' + (params != videoparams ? '' : '-')  + 'video+' + encodeURIComponent(cleanparams),
        json: true
      }, (err, res, body) => {
        if (err || res.statusCode != 200) return console.error(err || body)
        if (!body.items || body.items.length < 1) return this.sendMessage('Keine Ergebnisse /elo')
        const gifURL = body.items[Math.floor(Math.random() * body.items.length)]
        if (cleanparams.length < 1 && params != videoparams) {
          this.addNetzm('https://img.pr0gramm.com/' + gifURL.image, (videoparams != cleanparams), user)
          this.sendMessage('Zufälliges Video von pr0gramm.com addiert')
        }
        else {
          if (params != videoparams) pr0 = gifURL.image
          this.sendByFilter(imageHtml('https://thumb.pr0gramm.com/' + gifURL.thumb, 'https://img.pr0gramm.com/' + gifURL.image), true)
        }
      })
    },
    ...
    anagramde: function(user, params, meta) {
      params = params.toLowerCase().trim()
      if (params.length > 17) this.sendMessage('Nur 17 Zeichen /elo')
      request.post({
        url: 'http://www.sibiller.de/anagramme/cgi-bin/CallWP.cgi',
        form: {

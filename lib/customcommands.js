    ...
    lauer: function(user, params, meta) {
      ...
      request({
        ...
        const getPic = thread => {
          ...
          if (/^\.(?:jpe?g|gif|png|bmp)$/.test(thread.ext)) return '<a class="bild" href="' + file + '" target="_blank"><img class="image-embed-small" src="' + file + '" /></a>'
          const thumb = siteurl + board + '/thumb/' + thread.tim + '.jpg'

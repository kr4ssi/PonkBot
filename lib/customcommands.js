...
module.exports = {
  handlers: {
    ...
    pr0: function(user, params, meta) {
      request({
        url: 'https://pr0gramm.com/api/items/get?tags=!-video+' + encodeURIComponent(params),
        json: true
      }, (err, res, body) => {
        if (err || res.statusCode != 200) return console.error(err || body)
        if (!body.items || body.items.length < 1) return this.sendMessage('Keine Ergebnisse /elo')
        const gifURL = body.items[Math.floor(Math.random() * body.items.length)].image
        this.sendMessage('https://img.pr0gramm.com/' + gifURL + `.pic`)
      })
    },
    pr0vid: function(user, params, meta) {
      ...
      request({
        ...
        pr0 = gifURL.image
        this.sendByFilter(imageHtml('https://thumb.pr0gramm.com/' + gifURL.thumb, 'https://img.pr0gramm.com/' + pr0), true)
      })
    },

  ...
  handlers: {
    ...
    lauer: function(user, params, meta) {
      ...
      request({
        ...
        const getPic = thread => {
          const file = siteurl + board + '/src/' + thread.tim + thread.ext
          let thumb
          if (thread.ext === '.gif') return '<a class="bild" href="' + file + '" target="_blank"><img class="image-embed-small" src="' + file + '" /></a>'
          else if (/^\.(?:jpe?g|png|bmp)$/.test(thread.ext)) thumb = siteurl + board + '/thumb/' + thread.tim + '.png'
          else if (/^\.(?:mp4|webm)$/.test(thread.ext)) thumb = siteurl + board + '/thumb/' + thread.tim + '.jpg'
          else if (/^\.(?:mp3|ogg|flac|opus)$/.test(thread.ext)) thumb = siteurl + 'static/audio.png'
          else if (/^\.(?:7z|zip)$/.test(thread.ext)) thumb = siteurl + 'static/zip.png'
          else thumb = siteurl + 'static/file.png'
          //if (/^\.(?:mp4|flv|webm|og[gv]|mp3|mov|m4a)/.test(thread.ext)) /überlege
          return '<a class="bild" href="' + file + '" target="_blank"><img class="image-embed-small" src="' + thumb + '" /></a>'
        }
        ...
        if (thread.sub) this.sendByFilter('<span class="lauersubject">' + thread.sub + '<span>', true)

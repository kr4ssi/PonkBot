  ...
  handlers: {
    ...
    pr0: function(user, params, meta) {
      ...
      if (cleanparams.length < 1 && pr0) {
        ...
        url: 'https://pr0gramm.com/api/items/get?tags=!' + (params === novidparams ? '-' : '')  + 'video+' + encodeURIComponent(cleanparams),
        json: true
      }, (err, res, body) => {
        if (err || res.statusCode != 200) return console.error(err || body)
        ...
        if (params === novidparams) return this.sendMessage('https://img.pr0gramm.com/' + gifURL.image + '.pic')
        if (cleanparams.length < 1) {
          this.addNetzm('https://img.pr0gramm.com/' + gifURL.image, (novidparams != cleanparams), user)
    ...
    help: function(user, params, meta) {
      console.log(this.commands.helpdata.mute)
      if (this.commands.helpdata.hasOwnProperty(params)) this.sendMessage(this.commands.helpdata[params].synop)

  ...
  handlers: {
    ...
    pr0: function(user, params, meta) {
      ...
      if (params.length < 1 && pr0) {
        ...
        console.log(body)
        if (!video) return this.sendMessage('https://img.pr0gramm.com/' + item.image + '.pic')
        if (params.length < 1) [...Array(meta.repeat)].forEach((c, i) => {
    ...
    emote: function(user, params, meta) {
      ...
      .then(result => {
        ...
        this.sendMessage('Emote ' + params + ' wurde ' + count + ' mal pfostiert' + byuser + (width > 0 ? ('. Breite: ' + width) : '') + (height > 0 ? ('. Höhe: ' + height) : ''))
      })
    },
    emotesize: function(user, params, meta) {
      ...
      let width = split.join().trim()
      if (!/\d{1,3}/.test(width)) return this.sendMessage('Ist keine Zahl')
      this.db.knex('emotes').insert({ emote }).catch(() => {
        return this.db.knex('emotes').where({ emote }).update({ width })
      }).then(() => {
    ...
    lauer: function(user, params, meta) {
      ...
      this.fetch(siteurl + board + '/res/' + thread + '.json', {
        ...
        const post = body.find(posts => posts.no == postid)
        const getPic = post => {
          const file = board + '/src/' + post.tim + post.ext
          let thumb = board + '/thumb/' + post.tim
          if (post.ext === '.gif') thumb = file
        ...
        let pics
        if (post.tim) pics = getPic(post)
        if (post.extra_files) post.extra_files.forEach(pic => pics += ' ' + getPic(pic))
        if (post.sub && !/ pics$/.test(params)) this.sendByFilter('<span class="lauersubject">' + post.sub + '<span>', true)
        if (pics) this.sendByFilter(pics, true)
        if (post.com && !/ pics$/.test(params)) {
          let text = post.com.replace(/href="\//g, 'href="' + siteurl).replace(/class="quote"/g, 'class="greentext"')

  ...
  handlers: {
    ...
    wiki: function(user, params, meta) {
      request({
        ...
        if (err || res.statusCode != 200) return console.error(err || body)
        this.sendByFilter((body.thumbnail ? `<img class="fikuimage" src="${body.thumbnail.source}" />` : '') +
        `<a href="${body.content_urls.desktop.page}" target="_blank" rel="noopener noreferrer">${body.content_urls.desktop.page}<div></div></a>`, true)
        this.sendByFilter('<div class="fikuinfo">' + body.extract_html + '</div>', true)
      })
    }
  }
}

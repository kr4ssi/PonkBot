...
module.exports = {
  handlers: {
    ...
    wiki: function(user, params, meta) {
      request({
        url: 'https://de.wikipedia.org/api/rest_v1/page/summary/' + params,
        json: true
      }, (err, res, body) => {
        if (err || res.statusCode != 200) return console.error(err || body)
    ...
    pic: function(user, params, meta) {
      ...
      if (!url) return this.sendMessage('Ist keine Elfe /pfräh')
      if (/https:\/\/(?:www\.)?instagram\.com\/p\/[\w-]+\/?/i.test(url)) {
        Insta.getMediaInfoByUrl(url).then(info => {
          this.addLastImage(info.thumbnail_url.replace(/^http:\/\//i, 'https://')).then(image => {
            this.sendMessage(image + '.pic')
          })
        }).catch(err => this.sendMessage(err.message))
      }
      else if (/https:\/\/prnt\.sc\/\w+/i.test(url)) {
        request(url, (err, res, body) => {
          ...
          const regMatch = body.match(/https:\/\/image\.prntscr\.com\/image\/[^"]/i)
          if (!regMatch) {
            this.sendMessage('Fehler')
            console.error(body)
            return
          }
          this.sendMessage(regMatch[0])
        })
      }
    }
  }
...
function logoHintergrund(user, params, { command }) {

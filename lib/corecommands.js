  ...
  this.checkPermission({
    ...
    const checkPermission = (user) => {
      return new Promise((resolve)=>{
        this.checkPermission({ user, rank: 3, hybrid: 'mute' }).then(
          ()=>{ resolve(true) },
          ()=>{ resolve(false) }
        );
      });

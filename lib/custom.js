...
module.exports = {
  checkMessageTrigger: function(user, message) {
    if (![
      ...
      Object.keys(emotes).forEach(emote => {
        ...
        this.db.knex('emotescount').insert({ emote, count, user}).catch(() => {
  ...
  fetch: function (url, { qs = {}, form = {}, method = 'get', json = true, getlist = false, getrandom = false, customerr = [] } = {}) {
    return new Promise((resolve, reject) => {
      console.log('Fetch:', url, qs, form, method, json, getlist, getrandom, customerr)
      if (getlist && !json) return console.error('json must set to true')
  ...
  add: function(url, title, { user, willkür, fiku }) {
    ...
    const checkregex = host => new RegExp('^https?:\\/\\/([-\\w]+\\.)*' + host + '\\/.+', 'i').test(url)
    const sendJson = (manifest, cache) => {
      ...
      if (needUserScript.some(checkregex)) {
    ...
    if (url.match(/https?:\/\/(?:www\.)?nxload\.com\/(?:embed-)?(\w+)/i)) nxLoad()
    else if (/.*\.m3u8$/.test(url)) getDuration(manifest).then(sendJson)
    else if (allowedHosts.some(checkregex)) {
      execFile('youtube-dl', ['--dump-json', '-f', 'best', '--restrict-filenames', url], {maxBuffer: 10485760}, (err, stdout, stderr) => {
        if (err) {
        ...
        if (needManifest.some(checkregex)) {
          ...
          manifest.sources[0].url = manifest.sources[0].replace(/^http:\/\//i, 'https://')
          manifest.duration = info.duration
          getDuration(manifest, info).then(sendJson)
        }
        else this.addNetzm(info.url.replace(/^http:\/\//i, 'https://'), willkür, user, 'fi', title, url)
      })
    }
    else if (fiku) {

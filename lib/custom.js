  ...
  add: function(url, title, { user, willkür, fiku }) {
    ...
    const sendJson = (manifest, cache) => {
      ...
      if (host.needUserScript) {
        manifest.sources[0].url = this.server.weblink + '/redir?url=' + url.replace(/https:\/\/(openload.co|oload\.[a-z0-9-]{2,})\/(f|embed)\//, 'https://openload.co/f/').replace(/https:\/\/(streamango\.com|fruithosts\.net)\/(f|embed)\//, 'https://streamango.com/f/')

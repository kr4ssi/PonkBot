...
module.exports = {
  ...
  add: function(user, params, meta, fiku) {
    ...
    if (this.getUserRank(user) > 0 ) {
      ...
      if (url) {
        url = url.replace('https://synch-api.herokuapp.com/add.json?url=', '')
        const urlbak = url
        ...
        if (/.*\.m3u8$/.test(url) || allowedHosts.some(allowed => new RegExp('^https?:\\/\\/([-\\w]+\\.)*' + allowed + '\\/.+', 'i').test(url))) {
          const link = 'https://synch-api.herokuapp.com/add.json?url=' + url + (title ? '&title=' + encodeURIComponent(title) : '')
          ...
          request({url: link, json: true}, (err, res, body) => {
            ...
            if (needUserScript.some(allowed => new RegExp('^https?:\\/\\/([-\\w]+\\.)*' + allowed + '\\/.+', 'i').test(url))) this.client.createPoll({
              ...
              opts: ['https://github.com/kr4ssi/synch-api/raw/master/ks.user.js (update vom 3.3.19)', 'dann ' + urlbak + ' öffnen, ok drücken und player neu laden'],
              obscured: false
            })
          })
        }

...
module.exports = {
  ...
  pushToGit: function(filename, content, encoding) {
    ...
    return new Promise((resolve, reject) => {
      gitclient.RepositoryFiles.edit(...gitArr).then(result => {
        ...
        if (err.response && err.response.status == 400 && err.description === 'A file with this name doesn\'t exist') {
  ...
  sendByFilter: function(message, force) {
    ...
    for (let i = 0; i < count; i++) {
      filterstrings.push('###' + Math.random().toString(36).slice(2) + '###')
      console.log('Bot filter' + (i > 0 ? (' ' + (i + 1)) : ''))
      this.client.socket.emit('updateFilter', {
        name: 'Bot filter' + (i > 0 ? (' ' + (i + 1)) : ''),
        source: filterstrings[i],
        replace: message.substr(i * limit, limit),
        flags: '',
        active: true
      })
    }
    this.sendMessage(filterstrings.join(''))
    filterstrings.forEach((s, i) => this.client.socket.emit('updateFilter', {
      name: 'Bot filter' + (i > 0 ? (' ' + (i + 1)) : ''),
      source: '',
      replace: '',
      flags: ''

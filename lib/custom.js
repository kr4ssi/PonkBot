      ...
      return new Promise((resolve, reject) => {
        ...
        const tryToGetDuration = err => {
          ...
          if (tries > 1) {
            ...
            return this.sendMessage(err.message)
          }
          tries++
      ...
      const getInfo = (url, manifest) => {
        return new Promise((resolve, reject) => {
          const video = youtubedl(url, ['-U', '--restrict-filenames']).on('info', info => {
            ...
            if (info._duration_raw || manifest.live) {
              manifest.duration = info._duration_raw
              resolve(manifest)
              video.destroy()
            }
            else getDuration(manifest, video).then(resolve)//.finally(() => video.destroy())
          }).on('error', reject)
        })
      }
      getInfo(url, manifest).then(sendJson, ytDlErr)
    }
    else if (fiku) {

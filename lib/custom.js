        ...
        const sendJson = (manifest, cache) => {
          ...
          if (needUserScript.some(allowed => new RegExp('^https?:\\/\\/([-\\w]+\\.)*' + allowed + '\\/.+', 'i').test(url))) {
            ...
            this.client.createPoll({
              ...
              opts: ['Geht nur mit Userscript', this.server.weblink + '/ks.user.js (update vom 20.4.19)', 'dann ' + urlbak + ' öffnen', 'Ok klicken und falls es schon läuft player neu laden'],
              obscured: false
            })
          }

...
class PonkDatabase extends EventEmitter {
    ...
    createTableIfNotExists(tableName, callback) {
        ...
        return schema.hasTable(tableName).then(exists => {
            if (exists) return new Promise((resolve, reject) => {
              resolve()
            })
            else return schema.createTable(tableName, table => {
                callback(table)
            },(error)=>{
    ...
    createTables(){
        this.logger.log('Creating Tables');
            ...
            this.createTableIfNotExists('users', (table)=>{
                ...
                table.bigint('last_seen').unsigned();
            })

            // Chat history
            .then(() => this.createTableIfNotExists('chat', (table)=>{
                table.increments();
            ...
            // Videos
            .then(() => this.createTableIfNotExists('videos', (table)=>{
                table.string('type', 20);
                table.string('id');
                table.string('title');
                table.integer('duration_ms').unsigned();
                table.bigint('last_played').unsigned();
                table.integer('flags').unsigned();
            ...
            // Playlist statistics
            .then(() => this.createTableIfNotExists('video_stats', (table)=>{
                table.string('type', 20);
                table.string('id');
                table.string('user', 20);
            ...
            .then(() => this.createTableIfNotExists('user_count', (table)=>{
                table.bigint('timestamp').unsigned();
            ...
            .then(() => this.createTableIfNotExists('simplestore', (table)=>{
    ...
    updateTables(){
        ...
        this.getKeyValue('dbversion').then((version) => {
            ...
            if (this.version === state) {
                ...
                return this.createTableIfNotExists('webauth', (table) => {
                    table.increments();
                    table.string('user', 20)
            ...
            if (this.version === state) {
                ...
                return this.createTableIfNotExists('sessionstore', (table) => {
            ...
            if (this.version === state) {
                ...
                return this.createTableIfNotExists('users_meta', (table) => {
            ...
            state++; // 3
            if (this.version === state) {
              this.logger.log('Creating table for fiku-system.')
              return this.createTableIfNotExists('fiku', (table) => {
                table.increments();
            ...
            state++; // 4
            if (this.version === state) {
                this.logger.log('Creating table for emote counting.')
                return this.createTableIfNotExists('emotescount', (table) => {
            ...
            state++; // 5
            if (this.version === state) {
                this.logger.log('Creating table for last images.')
                return this.createTableIfNotExists('lastimage', (table) => {
                    table.increments();
            ...
            state++; // 6
            if (this.version === state) {
                this.logger.log('Creating table for netzms.')
                return this.createTableIfNotExists('netzms', (table) => {
                  table.increments();
                    table.string('faden', 240)
                    table.integer('nr');
                    table.string('netzm', 240)
                })
                .then(()=>{
                    this.setKeyValue('dbversion', ++state).then(()=>{
                        this.updateTables();

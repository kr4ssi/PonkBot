      ...
      this.bot.fetch('https://www.ardmediathek.de/ard/live/Y3JpZDovL2Rhc2Vyc3RlLmRlL0xpdmVzdHJlYW0tRGFzRXJzdGU', {
        ...
        Promise.all(stations.map((i, e) => {
          ...
          return addition.getInfo()//.then(() => Object.assign(addition, {
            ...
            const addition = this.bot.API.add.add(url, null, { gettitle: true })
            return addition.getInfo()
            //.then(() => Object.assign(addition, {
    ...
    handlers: {
      ...
      doku(user, params, meta) {
        this.fetch('https://mediathekviewweb.de/api/query', {
          ...
          this.API.add.cmAdditions[encodeURIComponent(body.url_website)] = {
            manifest: {
              title,
              live: false,
              duration: body.duration,
              sources: [body.url_video_low, body.url_video, body.url_video_hd].reduce((acc, url, i) => {
                if (!url) return acc
                return acc.concat({
                  url,
          ...
          this.mediaSend({ type: 'cm', id: this.server.weblink + '/add.json?' + 'url=' + encodeURIComponent(body.url_website), pos: meta.addnext ? 'next' : 'end' })
